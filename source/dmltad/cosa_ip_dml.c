    ...
    )
{
    ...
    if (!pValue || !pMyObject || !ParamName)
        return FALSE;

    /* Mirror former GetParamUlongValue: CosaDmlDiagGetResults only (no live g_Diag* prefer). */
    if (AnscEqualString(ParamName, "TestBytesReceived", TRUE))
    {
        ...
        if ( pDownloadDiagStats )
        {
            *pValue = (unsigned long long)pDownloadDiagStats->TestBytesReceived;
        }
        else
        {
            AnscTraceWarning(("Download Diagnostics---Failed to get TestBytesReceived!\n"));
    ...
    if (AnscEqualString(ParamName, "TotalBytesReceived", TRUE))
    {
        ...
        if ( pDownloadDiagStats )
        {
            *pValue = (unsigned long long)pDownloadDiagStats->TotalBytesReceived;
        }
        else
        {
            AnscTraceWarning(("Download Diagnostics---Failed to get TotalBytesReceived!\n"));
    ...
    )
{
    ...
    if (!pValue || !pMyObject || !ParamName)
        return FALSE;
    pUploadInfo = pMyObject->hDiagUploadInfo;

    /* Mirror former GetParamUlongValue TestFileLength: GetConfigs first, then hDiagUploadInfo. */
    if (AnscEqualString(ParamName, "TestFileLength", TRUE))
    {
        PDSLH_TR143_UPLOAD_DIAG_INFO pBbhmConfig =
            (PDSLH_TR143_UPLOAD_DIAG_INFO)CosaDmlDiagGetConfigs(DSLH_DIAGNOSTIC_TYPE_Upload);
        if ( pBbhmConfig )
        {
            *pValue = (unsigned long long)pBbhmConfig->TestFileLength;
            AnscFreeMemory(pBbhmConfig);
        }
        else if( pUploadInfo )
        {
            *pValue = (unsigned long long)pUploadInfo->TestFileLength;
        }
        else
        {
    ...
    if (AnscEqualString(ParamName, "TestBytesSent", TRUE))
    {
        ...
        if ( pUploadDiagStats )
        {
            *pValue = (unsigned long long)pUploadDiagStats->TestBytesSent;
        }
        else
        {
    ...
    if (AnscEqualString(ParamName, "TotalBytesSent", TRUE))
    {
        ...
        if ( pUploadDiagStats )
        {
            *pValue = (unsigned long long)pUploadDiagStats->TotalBytesSent;
        }
        else
        {
    ...
    )
{
    ...
    /* TestFileLength -> UploadDiagnostics_GetParamUint64Value */
    ...
    )
{
    ...
    /* TestFileLength -> UploadDiagnostics_SetParamUint64Value */
...
/**********************************************************************
    ...
    description:
        ...
        This function is called to set uint64/unsignedLong parameter value;
    ...
    )
{
    ...
    if (ParamName && AnscEqualString(ParamName, "TestFileLength", TRUE))
    {
        if (uValue == 0)
            return FALSE;

        /* Mirror former SetParamUlongValue TestFileLength (no DiagnosticsState=None). */
        pUploadInfo->TestFileLength = (ULONG64)uValue;
        /* Keep Bbhm in sync so getv (reads Bbhm via GetConfigs) sees ACS/dmcli sets */
        if (g_DiagUploadObj)
            g_DiagUploadObj->UploadDiagInfo.TestFileLength = (ULONG64)uValue;
        AutoTfl_MarkManual((ULONG64)uValue);
        return TRUE;
    }
