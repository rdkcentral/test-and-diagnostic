...
2601:9c0:280:4cb0::/64
*/
#if defined(_HUB4_PRODUCT_REQ_) || defined(_SR213_PRODUCT_REQ_) || defined(_SCER11BEL_PRODUCT_REQ_) || defined(_SCXF11BFL_PRODUCT_REQ_) || defined(_XER2_PRODUCT_REQ_)
	...
	if((!xNetSniffer_PID1)||(!xNetSniffer_PID2))  
	{
		...
		if((IPv4PID!=xNetSniffer_PID1)&&(IPv4PID!=xNetSniffer_PID2))
		{
			if(IPv4Enable==true)// check IPv4Enable enable is enable start the xNetSniffer service with IPv4
			{
				...
				int service_status = CheckLatencyMeasurementServiceStatus(SNIFFER_SERVICE,ServicePID);
				CcspTraceInfo(("%s: SNIFFER_SERVICE status=%s ServicePID='%s'\n",
						__FUNCTION__,
				...
				if (service_status != SERVICE_ACTIVE)
				{
...
char *save_ptr = NULL;
char *pid1 = strtok_r(pid_copy, " \t\r\n", &save_ptr);
char *pid2 = strtok_r(NULL, " \t\r\n", &save_ptr);
					...
					if (pid2 && (IPV6PID == safe_atoi(pid2)))
					{
					    IPv4PID = safe_atoi(pid1);
					}
					else if (pid1 &&
					     (IPV6PID == safe_atoi(pid1)))
					{
					    IPv4PID = safe_atoi(pid2);
					}
					else
					{
