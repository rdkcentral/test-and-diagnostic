...
#define WEBCFG_DECODER_BUF_SIZE (64 * 1024)

static char *read_pipe_data(FILE *pipe) {
    char *data = malloc(WEBCFG_DECODER_BUF_SIZE);
    if (!data) {
        return NULL;
    }

    size_t total = fread(data, 1, WEBCFG_DECODER_BUF_SIZE - 1, pipe);

    if (ferror(pipe)) {
        CcspTraceError(("read_pipe_data: fread error\n"));
        free(data);
        return NULL;
    }

    if (!feof(pipe)) {
        CcspTraceError(("read_pipe_data: output exceeded buffer (%d bytes), truncated\n",
                        WEBCFG_DECODER_BUF_SIZE));
        free(data);
        return NULL;
    }
...
static cJSON *Load_WebcfgDB_Array(void) {
    ...
    FILE *pipe = popen("webcfg_decoder -m /nvram/webconfig_db.bin", "r");
    if (!pipe) {
