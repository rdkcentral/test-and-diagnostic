...
/*
 * After an upload attempt: lock TFL, abort, or ScheduleNext another train run.
 * Returns TRUE if another internal run was started (ACS/WebPA must NOT see
 * Completed/Error yet — only one external Completed after TFL is locked).
 */
BOOL AutoTfl_Finish
    (
...
/* TRUE while ScheduleNext has armed StartDiag (suppress Requested notify). */
BOOL AutoTfl_IsInternalResume(void);
