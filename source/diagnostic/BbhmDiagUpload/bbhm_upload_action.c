...
updateTestFileLength(PDSLH_TR143_UPLOAD_DIAG_INFO pUploadInfo)
{
    ULONG64     uploadSpeedKbps = 0;
    ULONG64     uploadSpeedBps  = 0;
    uint64_t    testFileLength  = 0;
    ...
    if (ANSC_STATUS_SUCCESS == sruStatus && uploadSpeedKbps > 0)
    {
        ...
        CcspTraceInfo(("updateTestFileLength: got UploadSpeed %" PRIu64 " bps based on SRU DM\n", (uint64_t)uploadSpeedBps));

        testFileLength = (((uint64_t)uploadSpeedBps) * 3) / 4;
        ...
        CcspTraceInfo(("updateTestFileLength: TestFileLength=%" PRIu64 " bytes\n", pUploadInfo->TestFileLength));
        return;
    }
    ...
    )
{
    ...
    ULONG64                         initialTxBytes     = 0;
    ULONG64                         finalTxBytes       = 0;
    ...
    /*
     * AutoTFL ScheduleNext re-enters StartDiag while ACS still waits for one
     * external Completed — do not emit another Requested notify.
     */
    if (!AutoTfl_IsInternalResume())
    {
	...
	ret = checkIfTheURLIsValid(aSocket, buffer, AnscSizeOfString(buffer), &redirectionUrl);
    ...
    CcspTraceWarning(("initialTxBytes = %" PRIu64 "\n", (uint64_t)initialTxBytes));
     ...
     */
    {
        ...
        while (uBytesSent > 0)
        {
            ...
            send_size = (uBytesSent >= UPLOAD_SINGLE_BUFFER_SIZE) ? UPLOAD_SINGLE_BUFFER_SIZE : (ULONG)uBytesSent;
            ...
            pStats->TestBytesSent += (uint64_t)s_result;
            uBytesSent -= (uint64_t)s_result;
        }
    }
    ...
    CcspTraceInfo(("finalTxBytes = %" PRIu64 "\n", (uint64_t)finalTxBytes));
    ...
    else if (pStats->TestBytesSent > 0)
    {
         ...
         */
        CcspTraceWarning(("Interface BytesSent decreased (wrap/reset); using TestBytesSent for TotalBytesSent (initial %" PRIu64 " final %" PRIu64 ").\n",
                (uint64_t)initialTxBytes, (uint64_t)finalTxBytes));
        pStats->DiagStates = DSLH_TR143_DIAGNOSTIC_Completed;
    }
    else
    {
        CcspTraceError(("Interface stats invalid (finalTxBytes %" PRIu64 " < initialTxBytes %" PRIu64 ")\n", (uint64_t)finalTxBytes, (uint64_t)initialTxBytes));
    ...
    if (pStats->DiagStates == DSLH_TR143_DIAGNOSTIC_Completed)
    {
        ...
        CcspTraceWarning(("TotalTxBytes = %" PRIu64 " TestBytesSent = %" PRIu64 "\n",
            (uint64_t)pStats->TotalBytesSent, (uint64_t)pStats->TestBytesSent));
    }
    ...
    /* clear flags before AutoTfl_Finish (ScheduleNext needs idle upload) */
    pMyObject->bUpDiagOn         = FALSE;

    /*
     * AutoTFL may ScheduleNext another train run. Suppress ACS/WebPA Completed
     * (and Error if shrinking) until TFL is locked — only one external Completed.
     */
    if (AutoTfl_Finish(pMyObject, pStats))
    {
        CcspTraceInfo(("UploadDiag AutoTfl: suppress DiagnosticsState notify; training continues\n"));
        pStats->DiagStates = DSLH_TR143_DIAGNOSTIC_Requested;
        pMyObject->UploadDiagInfo.DiagnosticsState = DSLH_TR143_DIAGNOSTIC_Requested;
        pMyObject->bUpNotifyNeeded = FALSE;
    }
    else if (pStats->DiagStates == DSLH_TR143_DIAGNOSTIC_Completed)
    {
