...
/**********************************************************************
    ...
    Training (on each upload Requested):
      ...
      - Intermediate ScheduleNext must not emit ACS/WebPA Completed (AutoTfl_Finish
        returns TRUE → upload action suppresses DiagnosticsState notify)
 ...
 */
static uint64_t
NextBytes(uint64_t bytes, DOUBLE measuredSec, ULONG windowSec)
{
    DOUBLE             target;
    DOUBLE             safeSec;
 ...
 */
static ANSC_STATUS
ScheduleNext(PBBHM_UPLOAD_DIAG_OBJECT pObj, uint64_t bytes)
{
...
LockSize(uint64_t bytes)
{
    ...
    g_autoTfl.bResume     = FALSE;
    g_autoTfl.runsCompleted = 0;
    CcspTraceInfo(("UploadDiag AutoTfl: trained TestFileLength=%" PRIu64 "\n", g_autoTfl.storedBytes));
}

/* TRUE if another train run was scheduled. */
static BOOL
RetrySmaller(PBBHM_UPLOAD_DIAG_OBJECT pObj, uint64_t curBytes)
{
    uint64_t next;
    ...
    next = ClampBytes(curBytes / 2U);
    if (next >= curBytes)
        next = UPLOAD_TFL_DEFAULT_BYTES;

        CcspTraceWarning(("UploadDiag AutoTfl: shrink %" PRIu64 " -> %" PRIu64 " after failure, re-Request\n",
            curBytes, next));
...
void
AutoTfl_MarkManual(uint64_t testFileLength)
{
    ...
    g_autoTfl.bActive = FALSE;
    CcspTraceInfo(("UploadDiag AutoTfl: TFL=%" PRIu64 " manual (skip auto TFL)\n", testFileLength));
}
...
AutoTfl_TryManual(PDSLH_TR143_UPLOAD_DIAG_INFO pUploadInfo)
{
    ...
    g_autoTfl.bActive = FALSE;
    g_autoTfl.bResume = FALSE;
    CcspTraceInfo(("UploadDiag AutoTfl: use manual TFL=%" PRIu64 "\n", pUploadInfo->TestFileLength));
    return TRUE;
}
...
AutoTfl_Start(PDSLH_TR143_UPLOAD_DIAG_INFO pUploadInfo)
{
    ...
    if (g_autoTfl.bDone && g_autoTfl.storedBytes >= UPLOAD_TFL_DEFAULT_BYTES)
    {
        ...
        CcspTraceInfo(("UploadDiag AutoTfl: reuse TFL=%" PRIu64 "\n", g_autoTfl.storedBytes));
        return;
    }
    ...
    runNo = g_autoTfl.runsCompleted + 1;
        CcspTraceInfo(("UploadDiag AutoTfl: run %lu/%d starting, TFL=%" PRIu64 "\n",
            (unsigned long)runNo, AUTOTFL_MAX_ROUNDS,
            (uint64_t)pUploadInfo->TestFileLength));

    g_autoTfl.bResume = FALSE;
}

BOOL
AutoTfl_IsInternalResume(void)
{
    return g_autoTfl.bResume;
}
    ...
    )
{
    ...
    bytes = (uint64_t)pMyObject->UploadDiagInfo.TestFileLength;
    ...
    if (state != DSLH_TR143_DIAGNOSTIC_Completed)
    {
        ...
        if (canShrink && bytes > 0 && pStats->TestBytesSent * 10 >= bytes)
            return RetrySmaller(pMyObject, bytes);

        CcspTraceWarning(("UploadDiag AutoTfl: fail DiagState=%lu sent=%" PRIu64 "/%" PRIu64 " — abort "
                "(next Requested will retrain)\n",
                (unsigned long)state,
                (uint64_t)pStats->TestBytesSent,
                (uint64_t)bytes));
    ...
    CcspTraceInfo(("UploadDiag AutoTfl: run %lu/%d done, duration=%.3f s, TFL=%" PRIu64 "\n",
            (unsigned long)runNo, AUTOTFL_MAX_ROUNDS, sec, (uint64_t)bytes));
    ...
    if (g_autoTfl.runsCompleted + 1 >= (ULONG)AUTOTFL_MAX_ROUNDS)
    {
        ...
        CcspTraceInfo(("UploadDiag AutoTfl: last round outside band "
                "(%.3f s), lock adjusted TFL %" PRIu64 " -> %" PRIu64 "\n",
                sec, (uint64_t)bytes, (uint64_t)next));
        LockSize(next);
        return FALSE;
    }

    next = NextBytes(bytes, sec, window);
    /* Put %.3f before uint64 args: ILP32 CcspTrace varargs scramble float after PRIu64. */
    CcspTraceWarning(("UploadDiag AutoTfl: adjust TFL (duration=%.3f s) %" PRIu64 " -> %" PRIu64 ", re-Request\n",
            sec, (uint64_t)bytes, (uint64_t)next));
