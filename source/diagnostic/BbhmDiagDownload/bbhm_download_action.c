    ...
    )
{
    ...
    errno_t                         rc                 = -1;
    ULONG64                         initialRxBytes     = 0;
    ULONG64                         finalRxBytes       = 0;
    ...
    CcspTraceInfo(("initialRxBytes = %" PRIu64 "\n", (uint64_t)initialRxBytes));
    ...
    CcspTraceInfo(("finalRxBytes = %" PRIu64 "\n", (uint64_t)finalRxBytes));
    ...
    else if (pStats->TestBytesReceived > 0)
    {
         ...
         */
        CcspTraceWarning(("Interface BytesReceived decreased (wrap/reset); using TestBytesReceived for TotalBytesReceived (initial %" PRIu64 " final %" PRIu64 ").\n",
                (uint64_t)initialRxBytes, (uint64_t)finalRxBytes));
        pStats->DiagStates = DSLH_TR143_DIAGNOSTIC_Completed;
    }
    else
    {
        CcspTraceError(("Interface stats invalid (finalRxBytes %" PRIu64 " < initialRxBytes %" PRIu64 ")\n", (uint64_t)finalRxBytes, (uint64_t)initialRxBytes));
    ...
    if (pStats->DiagStates == DSLH_TR143_DIAGNOSTIC_Completed)
    {
        ...
        CcspTraceWarning(("TotalRxBytes = %" PRIu64 " TestBytesReceived = %" PRIu64 "\n",
            (uint64_t)pStats->TotalBytesReceived, (uint64_t)pStats->TestBytesReceived));
