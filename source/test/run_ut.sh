...
log "INFO" "Running make for CcspTandD_Dml_Test_gtest.bin..."
if make -C source/test/CcspTandD_Dml_Test; then
...
if [ -f "./source/test/CcspTandD_Dml_Test/CcspTandD_Dml_Test_gtest.bin" ]; then
    log "INFO" "Running CcspTandD_Dml_Test_gtest.bin"
    ./source/test/CcspTandD_Dml_Test/CcspTandD_Dml_Test_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "CcspTandD_Dml_Test_gtest.bin does not exist, cannot run tests"
    exit 1
fi
