	...
	then
		...
		then
			...
			if [ -n "$ula_gw" ]; then
				...
				if [ -n "$CHECK_PACKET_RECEIVED" ] && [ "$CHECK_PACKET_RECEIVED" -ne 100 ]; then
					ping6_success=1
					ping6_failed=0
				...
				do
					PING_OUTPUT=`ping6 -I "$WAN_INTERFACE" -c "$PINGCOUNT" -w "$RESWAITTIME" -s "$PING_PACKET_SIZE" "$gw_addr" 2>/dev/null`
					CHECK_PACKET_RECEIVED=`echo $PING_OUTPUT | grep "packet loss" | cut -d"%" -f1 | awk '{print $NF}'`
